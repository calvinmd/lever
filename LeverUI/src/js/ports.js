...
// The commented lines of code refer to methods in CompoundLens methods, whereas we are using methods in VortexLens ("c" vs. "v")
...
function subscribeToCTokenPorts(app, eth) {
  ...
  app.ports.askCTokenMetadataAllPort.subscribe(({ blockNumber, cTokens, compoundLens }) => {
    ...
    wrapCall(app, eth, [[CompoundLens, compoundLens, 'vTokenMetadataAll', [cTokens]]], blockNumber)
      .then(([results]) => {
        const cTokenMetadataList = results.map(
          ({
            // cToken: cTokenAddress,
            vToken: cTokenAddress,
  ...
  app.ports.askCTokenGetBalancesPort.subscribe(
    ({ blockNumber, customerAddress, cTokens: cTokenEntries, compoundLens }) => {
      ...
      wrapCall(
        ...
        [[CompoundLens, compoundLens, 'vTokenBalancesAll', [Object.keys(cTokens), customerAddress]]],
        blockNumber
      )
        .then(([results]) => {
...
function subscribeToComptrollerPorts(app, eth) {
  ...
  app.ports.askOraclePricesAllPort.subscribe(({ blockNumber, cTokens: cTokenEntries, compoundLens }) => {
    ...
    wrapCall(app, eth, [[CompoundLens, compoundLens, 'vTokenUnderlyingPriceAll', [Object.keys(cTokens)]]], blockNumber)
      .then(([results]) => {
          ...
          web3Eth
            ...
            .catch((e) => {
              ...
              if (e.code === 4001) {
                ...
                console.log(e);
                let txMessage = 'Transaction Rejected By User';
                app.ports.etherTransactionRejectedPort.send({
                  txModule,
                  txId,
                  txMessage
                ...
                let txMessage = 'Molecule Contract: NOT WHITELISTED ADDRESS';
                app.ports.etherTransactionRejectedPort.send({
                  txModule,
                  txId,
                  txMessage
