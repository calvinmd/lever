port module Eth.Transaction exposing
    ...
    , getRejectedTransactionsForAccount
    , giveNewTrx
        ...
        NewNonBNTransaction { txModule, txId, txHash } ->
            ...
            case maybeFoundBNTransaction of
                Just foundBNTransaction ->
                    ...
                    --convert BNTransaction to New Transaction (Transaction type)
    ...
    let
        ...
        decoder =
            Json.Decode.map5
                ...
                (field "message" (Json.Decode.field "txMessage" (Json.Decode.maybe Json.Decode.string)))
    in
    giveNewTrxPort
...
convertBNTransactionToTransaction bnTransaction =
    ...
    case ( bnTransaction.txHash, bnTransaction.timestamp ) of
        ( Just actualTXHash, Just actualTime ) ->
            ...
            , message = bnTransaction.txMessage
            }
                |> Just
...
getRejectedTransactionsForAccount : Maybe Network -> Account -> Maybe Time.Posix -> Maybe BNTransactionState -> List Transaction -> List Transaction
getRejectedTransactionsForAccount maybeNetwork account maybeOldestTrxTimestamp maybeBNTransactionState transactions =
    let
        rejectedBNTransactions =
            case maybeBNTransactionState of
                Just bnTransactionState ->
                    BNTransaction.getFailedBNTransactionsForAccount maybeNetwork account bnTransactionState
                        |> List.filterMap convertBNTransactionToTransaction

                _ ->
                    []
        allRejectedTransactions =
            rejectedBNTransactions ++ transactions
    in
    case ( maybeNetwork, account, maybeOldestTrxTimestamp ) of
        ( Just network, Acct customerAddress _, Just oldestTrxTimeStamp ) ->
            allRejectedTransactions
                |> List.filter (.status >> (==) Failure)
                |> List.filter (.network >> (==) network)
                |> List.filter (.customer >> (==) customerAddress)
                |> List.filter (\trx -> Time.posixToMillis trx.timestamp >= Time.posixToMillis oldestTrxTimeStamp)
