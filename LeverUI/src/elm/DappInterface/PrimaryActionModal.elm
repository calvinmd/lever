...
inputActionPane userLanguage maybeConfig maybeEtherUsdPrice primaryActionModalState mainModel =
    let
        ( tokenApproved, maybePendingTransaction, maybeFailedTransaction ) =
            case maybeConfig of
                Just config ->
                    ...
                    , getMostRecentAssetRejectedTransaction config primaryActionModalState mainModel
                    )
        ...
        --maybeFailedTransaction
            case (maybePendingTransaction, maybeFailedTransaction) of
                ( Just pendingTransaction, _) ->
                    ( div [ class "title" ]
                        [ span [] [ text (Translations.transaction_pending userLanguage) ]
                        -- [ span [] [ text (Maybe.withDefault "Text" pendingTransaction.message) ]
                        ]
                    , awaitingPendingTransactionView pendingTransaction primaryActionModalState mainModel
                    )

                (_, Just failedTransaction ) ->
                    ( div [ class "title" ]
                        [ span [] [ text "Failed Transaction" ]
                        ]
                    , failedTransactionView failedTransaction primaryActionModalState mainModel
                    )
                _ ->
...
failedTransactionView : Transaction -> PrimaryActionModalState -> Model -> Html Msg
failedTransactionView transaction { primaryActionType } { network, userLanguage } =
    let
        ( connectingRingTypeClass, buttonTypeClass ) =
            if primaryActionType == MintAction || primaryActionType == RedeemAction then
                ( ""
    ...
    div [ class "enable-asset extra-bottom-margin" ]
        [ div [ ]
            [ div [] [] ]
        , p [ class "small text-center" ]
            [ text (Maybe.withDefault "Failed Transaction" transaction.message) ]
        ]
...
getMostRecentAssetRejectedTransaction : Config -> PrimaryActionModalState -> Model -> Maybe Transaction
getMostRecentAssetRejectedTransaction config { chosenAsset } { currentTime, network, account, tokenState, transactionState, bnTransactionState } =
    let
        rejectedRecentTransactions =
            transactionState.transactions
                |> Eth.Transaction.getRejectedTransactionsForAccount network account (Eth.Transaction.getDefaultOldestPendingTrxTime currentTime) (Just bnTransactionState)
        -- Any transaction for the asset will block the whole input view.
        rejectedAssetTransactions =
            case ( network, account ) of
                ( Just actualNetwork, Acct customerAddress _ ) ->
                    Eth.Transaction.filteredTransactionsByCToken rejectedRecentTransactions config actualNetwork customerAddress tokenState.cTokens chosenAsset
